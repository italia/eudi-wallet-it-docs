...
- Other scenarios where the owner can no longer access their Wallet Instance.

If any of these situations occur, the owner **MUST** revoke the Wallet Instance.

.. note::

The *owner* is the end user who possesses the smartphone with the Wallet Instance and is responsible for managing its entire lifecycle. 

To achieve this the Wallet Provider (WP) **MUST** provide a Web Portal where the end user can access and request the revocation of the Wallet Instance.

The suggested solution for a owner is to establish a secure backup plan at the beginning of the registration process. The backup **MUST NOT** be stored in the same location as the private key. Instead, it should be saved to a different location, such as a cloud solution or a legacy storage solution. The purpose of the backup is to facilitate the recovery of the Hardware Private Key generated by the secure HSM chip on the smartphone's motherboard. For example, a method such as a the generation of seed phrases could be implemented to recover the lost Hardware private key.
In this manner, the owner can effortlessly restore their Wallet Instance without generating a new one.
