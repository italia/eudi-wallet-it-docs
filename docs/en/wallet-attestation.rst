...
- Other scenarios where the holder can no longer access their Wallet Instance.

If any of these situations occur, the holder **MUST** revoke the Wallet Instance.

.. note::

the *Holder* is defined as the user that has activated and has full control of an instance of the EUDI wallet and the related private keys
...
The suggested solution for a holder is to establish a secure backup plan at the beginning of the registration process. The purpose of the backup is to facilitate the recovery of the Hardware Private Key generated by the secure HSM chip on the smartphone's motherboard. For example, a method such as a the generation of seed phrases could be implemented to recover the lost Hardware private key.
In this manner, the holder can effortlessly restore their Wallet Instance without generating a new one.
