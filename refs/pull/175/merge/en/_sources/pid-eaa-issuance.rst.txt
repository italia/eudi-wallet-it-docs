...
The PID/(Q)EAA Provider performs the following checks upon the receipt of the PAR request:
    ...
    1. It MUST validate the signature of the Request Object using the algorithm specified in the ``alg`` header parameter (:rfc:`9126`, :rfc:`9101`) and the public key retrieved from the Wallet Instance Attestation (``cnf.jwk``) referenced in the Request Object, using the ``kid`` JWS header parameter.
...
Below an non-normative example of the Wallet Instance Attestation Proof of Possession without encoding and signature applied:
...
.. note::
   The PID/(Q)EAA Provider MUST validate the signature of the the Wallet Instance Attestation and that it is not expired.
...
The ``client_assertion`` is signed using the private key which the public key is referenced in the Wallet Instance Attestation (``cnf.jwk`` claim). The PID/(Q)EAA Provider performs the following checks:
   ...
   4. It MUST ensure the ``redirect_uri`` matches the value included in the previous Request Object `OpenID.Core#TokenRequest <https://openid.net/specs/openid-connect-core-1_0.html#TokenRequest>`_.
   5. It MUST validate the DPoP Proof JWT, according to (:rfc:`9449`) Section 4.3.
